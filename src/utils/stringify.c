...
const char* stringifyParameterType(ParameterType type) {
    switch (type) {
        ...
        STRINGIFY_ENUM_CASE(PV_T_FONT_WIDTH)
        STRINGIFY_ENUM_CASE(PV_T_FONT_FAMILY)
        STRINGIFY_ENUM_CASE(PV_T_FONT_WEIGHT)
        STRINGIFY_ENUM_CASE(PV_T_FONT_STYLE)
        STRINGIFY_ENUM_CASE(PV_T_TEXT_DECORATION)
        STRINGIFY_ENUM_CASE(PV_T_BACKGROUND_COLOR)
        default:
            return "Unknown";
    }
}
